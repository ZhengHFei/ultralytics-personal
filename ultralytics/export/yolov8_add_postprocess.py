...
def add_postprocess(onnx_model_path, size=8400, class_num=80):
    ...
    # input
    origin_output = [node for node in graph.nodes if node.name == "/model.22/Concat_5"][0]  # Concat_307是output的输入结点，不同的模型需要修改
    print(origin_output.outputs)

    output_t = gs.Variable(name="output_t",shape=(1, size, 4+class_num),dtype=np.float32)
    ...
    ends_wh = gs.Constant("ends_wh",values=np.array([1,size, 4],dtype=np.int64))
    ...
    # ends_object = gs.Constant("ends_object",values=np.array([1,size,5],dtype=np.int64))

    starts_conf = gs.Constant("starts_conf",values=np.array([0,0,4],dtype=np.int64))
    ends_conf = gs.Constant("ends_conf",values=np.array([1,size,4+class_num],dtype=np.int64))

    # output
    box_xywh_0 = gs.Variable(name="box_xywh_0",shape=(1,size,4),dtype=np.float32)
    label_conf_0 = gs.Variable(name='label_conf_0',shape=(1,size,class_num),dtype=np.float32)
    ...
    # identity
    box_xywh = gs.Variable(name="box_xywh",shape=(1,size,4),dtype=np.float32)
    label_conf = gs.Variable(name='label_conf',shape=(1,size,class_num),dtype=np.float32)
    ...
    ends_1 = gs.Constant("ends_x",values=np.array([1,size,1],dtype=np.int64))
    ...
    ends_2 = gs.Constant("ends_y",values=np.array([1,size,2],dtype=np.int64))

    starts_3 = gs.Constant("starts_w",values=np.array([0,0,2],dtype=np.int64))
    ends_3 = gs.Constant("ends_w",values=np.array([1,size,3],dtype=np.int64))

    starts_4 = gs.Constant("starts_h",values=np.array([0,0,3],dtype=np.int64))
    ends_4 = gs.Constant("ends_h",values=np.array([1,size,4],dtype=np.int64))

    # output
    x = gs.Variable(name="x_center",shape=(1,size,1),dtype=np.float32)
    y = gs.Variable(name="y_center",shape=(1,size,1),dtype=np.float32)
    w = gs.Variable(name="w",shape=(1,size,1),dtype=np.float32)
    h = gs.Variable(name="h",shape=(1,size,1),dtype=np.float32)
    ...
    # output
    w_ = gs.Variable(name="w_half_",shape=(1,size,1),dtype=np.float32)
    wplus = gs.Variable(name="w_half_plus",shape=(1,size,1),dtype=np.float32)
    h_ = gs.Variable(name="h_half_",shape=(1,size,1),dtype=np.float32)
    hplus = gs.Variable(name="h_half_plus",shape=(1,size,1),dtype=np.float32)
    ...
    #变换2
    # output
    x1 = gs.Variable(name="x1",shape=(1,size,1),dtype=np.float32)
    y1 = gs.Variable(name="y1",shape=(1,size,1),dtype=np.float32)
    x2 = gs.Variable(name="x2",shape=(1,size,1),dtype=np.float32)
    y2 = gs.Variable(name="y2",shape=(1,size,1),dtype=np.float32)
    ...
    boxes_0 = gs.Variable(name="boxes_0",shape=(1,size,4),dtype=np.float32)
    ...
    shapes = gs.Constant("shape",values=np.array([1,size,1,4],dtype=np.int64))

    # output
    boxes = gs.Variable(name="boxes",shape=(1,size,1,4),dtype=np.float32)
    ...
    # #----处理prob
    # scores = gs.Variable(name="scores",shape=(1,size,4),dtype=np.float32)
...
if __name__ == "__main__":
    ...
    onnx_path = r"D:\project\ultralytics\ultralytics\log\abrasion\train_small2\weights\last.onnx"
    graph = add_postprocess(onnx_path, size=8400, class_num=1)
    name = "abrasion_aug_small_640"
    onnx.save(gs.export_onnx(graph),f"./onnx/{name}_post.onnx")
